...
**[pepiros.vercel.app](https://pepiros.vercel.app)**
...
| **MCP server** (`mcp/*`) | All 12 tools in `lib/mcp/registry.ts` are live over stdio (`search_paper`, `verify_claim`, `get_outline`, `get_node`, `create_node`, `find_contradictions`, `paper_facts`, `list_papers`, `list_workspaces`, `create_workspace`, `add_paper`, `get_job`), plus 3 resource templates for `@`-mentioning a workspace/paper/node and the 4 `docs/PLAN-V1.md` §13.3 prompts. `create_node` re-verifies submitted evidence server-side, so a client cannot assert `quote_located`. A token requires a live session (or is refused outright in a production-configured process with none) rather than defaulting to unrestricted access. Verified over the real protocol with the SDK's own client. The remote streamable-HTTP transport (`app/api/mcp/route.ts`) is real too, with a full RFC 7591/6749 OAuth chain (dynamic client registration, PKCE, single-use authorization codes) for a hosted connector rather than a locally-run stdio process -- both the stdio and remote surfaces share the same token-pin/rate-limit checks, verified live (a workspace-pinned token can't reach another workspace, and hammering a tool or the OAuth endpoints past their limit gets a real 429). |
...
| [`docs/`](docs/) (Markdown and Word, same content in both) | Formal project documentation: overview and audience, installation and setup, user manual, technical reference (architecture, full data model, MCP tool reference) |
