...
| **Remaining generators** | 18 of the ~22 real types in `docs/PLAN-V1.md` §8 are implemented (`lib/agents/generators/`). Left: `figures` (needs a vision call against a cropped raster; `scripts/parse.py` doesn't extract figure crops yet), `equations` (needs equation-kind chunks with bbox anchoring; the parser only emits `prose` today), and `concept_links` (needs cross-paper context a single-paper `GeneratorContext` doesn't carry, plus a way to write real `relates` edges from a generator's output). `quiz`/`flashcards` are solved a different way -- `lib/services/quiz.ts` and `components/learn/FlashcardDeck.tsx` derive them from already-verified leaf nodes rather than being their own generator. |
...
| **MCP package publish** | `package.json` has a real `bin` (`pepiros-mcp`) and `build:mcp` (esbuild, bundles the tool layer to one file, `npm pack --dry-run` confirms a ~34KB tarball with no app source), but the package is still `private: true` and unpublished, so `npx pepiros-mcp` isn't live yet -- that's a maintainer decision (npm account, package-name availability), not a code gap. What's left otherwise is the remote streamable-HTTP + OAuth transport; `docs/PLAN-V1.md` §13.4 says verify connector requirements against Anthropic's current docs before building that. |
