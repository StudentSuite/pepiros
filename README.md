...
| **MCP, remaining** | All 12 tools in `lib/mcp/registry.ts` are registered (`npm run mcp:stdio`). What's left is the remote streamable-HTTP + OAuth transport -- `docs/PLAN-V1.md` §13.4 says verify connector requirements against Anthropic's current docs before building that -- plus a real `npx` install path (see "Export & promote" row: `pepiros-mcp` isn't published). |
