...
Still stubbed (see the `TODO` comment at the top of each file): PDF ingestion (`scripts/parse.py`, `scripts/ocr_fallback.py`, `app/api/ingest`), 15 of the 21 node generators plus figure vision, the MCP server (`mcp/*`), graph-mutation endpoints (`app/api/nodes/*`, `/api/compare`), and chat/export/auth. None of these have a live Supabase project or `GROQ_API_KEY` behind them yet.

### Added

- **The LLM layer now runs on Groq, not Anthropic** (`@ai-sdk/groq` + the Vercel AI SDK, model ids env-configurable via `GROQ_MODEL_FAST`/`GROQ_MODEL_STRONG` -- see `lib/ai/client.ts`). `ai` bumped 4.x -> 5.x to pair with a `@ai-sdk/groq` version sharing the same `@ai-sdk/provider` major (2.x); `@ai-sdk/anthropic`/`@anthropic-ai/sdk` removed.
