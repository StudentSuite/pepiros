...
A generator is instructed to cite a context-block id as the bare token ("C7"), not the full bracketed header ("C7 | Methods | p.4") — but this is a prompt request, not a guarantee, and a real model returned the full header live while this was built. `lib/agents/orchestrator.ts`'s `normalizeRef()` strips to the leading token before verification regardless of what the model actually returned, so a model ignoring the instruction still resolves correctly instead of silently registering as `hallucinated_ref`. Tests mock the model at the `LanguageModelV2` level (`lib/testing/mockLanguageModel.ts`) so none of `lib/agents/*.test.ts` needs a real API key.
