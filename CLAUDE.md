...
- **Design tokens only.** `app/globals.css` / `tailwind.config.ts` define the full "lab notebook at night" palette (`surface`/`paper`/`ink`/`border`/`pillar-1..7`/`located`/`paraphrase`/`unsupported`) and font families (`font-serif` for reading prose, `font-sans` for UI chrome, `font-mono` for citation ids). Don't introduce new colors or a light theme.
