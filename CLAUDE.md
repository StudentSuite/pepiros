...
`npm test` is Vitest over `lib/**/*.test.ts` (405 cases across 47 files): the grounding spine, layout, graph visibility, citation parsing, the service layer, and the LLM/chat agents, the last two mocked at the model level, so no API key is needed. It is a different thing from `evals/`, which runs a starting set of real golden test cases against the actual generator pipeline (structural invariants, not prose-quality judgment) and is still growing, not a stub. `npm run typecheck` plus `npm test` is the correctness bar. Run both after every change to `lib/`.
...
- **Design tokens only.** `app/globals.css` / `tailwind.config.ts` are the source of truth for the palette (`surface`/`paper`/`ink`/`border`/`accent`/`pillar-1..7`/`located`/`paraphrase`/`unsupported`/`inference`/`ungrounded`), the type scale, spacing, radius, elevation, motion, layout dims and stacking order. Font families: `font-serif` (Source Serif) for long-form reading prose only, `font-sans` (Geist) for all UI chrome and headings, `font-mono` (Geist Mono) for citation ids and the `.kicker` eyebrow. Don't introduce new colors, and don't use an arbitrary value (`text-[13px]`, `rounded-2xl`, `z-[60]`) where a token exists.
- **Both themes ship.** Light is the default `:root` block, dark is `.dark`, and `components/theme/ThemeToggle.tsx` is a three-way Light/System/Dark control. The "dark only, light theme is cut" line in `docs/PLAN-V1.md` §14.2 and §20 is superseded and marked as such; `app/globals.css` is the authority. Every new surface must be checked in both.
- **Which primitive system to use** (issue #350). Two ship deliberately:
  - `components/shadcn/` is canonical wherever the interaction semantics are hard to get right by hand: overlays (dialog, alert-dialog, sheet), dropdown-menu, popover, tooltip, command, sidebar. Radix gives focus trap, keyboard nav, Escape, focus restore and ARIA wiring for free, and hand-rolling those is how you ship a modal a keyboard user can tab out of.
  - `components/ui/` is canonical for brand primitives with no complex interaction: Button, Badge, EvidenceBadge, PillarChip, RefChip, Panel, Skeleton, EmptyState, ErrorBanner, FormField, Logo.
  - A vendored shadcn file must be repointed at project tokens before use. Shipping upstream defaults is how `animated-beam` ended up with `#ffaa40`/`#9c40ff` in it (#341) and the overlays with `bg-black/80` (#380).
  - Do not add a third implementation of something that already exists in either directory. The per-component reconciliation is #344-#349.
...
- **Cut list (docs/PLAN-V1.md §20), do not rebuild these**: pgvector/embeddings/BM25, elkjs/auto-layout (nodes carry deterministic `x`/`y` already), a deployed Python service (PyMuPDF/PaddleOCR run as local scripts only), `role="application"` on the canvas (breaks screen readers), minimap, SM-2 spaced repetition / adaptive quiz difficulty. **"Light theme" was on this list and is no longer: it shipped 2026-08-23 and is the default.**
