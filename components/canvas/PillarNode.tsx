...
export function PillarNode({ data }: NodeProps<PepirosNode>) {
  ...
  return (
    <div
      className={clsx(
        "w-52 rounded border-2 bg-surface-raised px-3 py-2 transition-opacity",
        node.stale && "opacity-50",
      )}
      style={{ borderColor: color, backgroundColor: `color-mix(in srgb, ${color} 8%, var(--surface-raised))` }}
    >
